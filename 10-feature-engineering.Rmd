...
basic_recipe <- recipe(Sale_Price ~ Lot_Area + Year_Built + Overall_Qual, 
                       data = ames_train)
...
enhanced_recipe <- recipe(Sale_Price ~ Lot_Area + Year_Built + Overall_Qual + 
...
scaling_recipes <- list(
  ...
  robust = recipe(~ ., data = scaling_demo) %>%
    step_normalize(all_predictors(), 
                  statistic = c("median", "mad"))  # Robust to outliers
)
...
thoughtful_recipe <- recipe(Sale_Price ~ ., data = ames_train) %>%
  ...
  step_interact(terms = ~ Gr_Liv_Area:Overall_Qual)  # Specific, meaningful interaction
```
...
exercise_recipe <- recipe(Sale_Price ~ ., data = ames_train) %>%
  ...
  step_mutate(
    ...
    Quality_x_Condition = Overall_Qual * Overall_Cond
  ) %>%
  ...
  # Interactions
  step_interact(terms = ~ Total_SF:Overall_Qual) %>%
...
In [Chapter 11](11-model-specification.qmd), we'll explore parsnip for unified model specification across different engines.
