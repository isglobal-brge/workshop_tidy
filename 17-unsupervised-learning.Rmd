...
clustering_data <- ames_numeric %>%
  select(Gr_Liv_Area, Overall_Qual, Year_Built, Total_Bsmt_SF, Garage_Area)
...
# Visualize clusters
p1 <- ggplot(clustered_data, aes(x = Gr_Liv_Area, y = Overall_Qual, 
                                  color = cluster)) +
...
p1 <- ggplot(comparison_data, aes(x = Gr_Liv_Area, y = Overall_Qual, 
                                   color = kmeans)) +
  geom_point(size = 2) +
  labs(title = "K-Means Clustering")

p2 <- ggplot(comparison_data, aes(x = Gr_Liv_Area, y = Overall_Qual, 
                                   color = hierarchical)) +
  geom_point(size = 2) +
...
gower_distance <- function(data) {
  ...
  mixed_data <- ames %>%
    select(Gr_Liv_Area, Overall_Qual, Neighborhood, Central_Air) %>%
    slice_sample(n = 100)
  ...
  mixed_data %>%
    ...
    ggplot(aes(x = Gr_Liv_Area, y = Overall_Qual, color = cluster)) +
    geom_point(size = 3) +
...
In [Chapter 18](18-model-deployment.qmd), we'll learn how to deploy models to production.
