...
params_with_data <- params %>%
  mutate(
    data = simulated_data,
    actual_mean = map_dbl(data, mean),
    actual_sd = map_dbl(data, sd)
  )
...
params_with_data %>%
  mutate(data = map2(data, distribution, ~ {
    tibble(value = .x, distribution = .y)
  })) %>%
  select(distribution, data) %>%
  unnest(data) %>%
...
# Parallel processing
system.time({
  parallel_results <- future_map_df(1:100, ~ large_simulation())
})
...
In [Chapter 7](07-strings-dates.qmd), we'll explore string manipulation and date/time handling with stringr and lubridate.
