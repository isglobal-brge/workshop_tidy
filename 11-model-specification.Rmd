...
# Show all available regression models
parsnip::show_model_info("regression")
```
...
ames_class <- ames %>%
  mutate(expensive = factor(if_else(Sale_Price > median(Sale_Price), 
                                    "yes", "no"))) %>%
  select(expensive, Gr_Liv_Area, Year_Built, Overall_Qual) %>%
  slice_sample(n = 500)
...
linear_comparisons <- map(engines_to_compare, function(eng) {
  ...
  if (eng == "glmnet") {
    spec <- spec %>% set_engine(eng, penalty = 0)  # No regularization
  } else {
    spec <- spec %>% set_engine(eng)
  }
  
  # Fit the model
  fit <- spec %>% fit(Sale_Price ~ Gr_Liv_Area + Overall_Qual, data = ames_small)
...
models <- list(
  linear = linear_reg() %>% 
    set_engine("lm") %>%
    fit(Sale_Price ~ Gr_Liv_Area + Overall_Qual, data = ames_small),
...
class_model <- logistic_reg() %>%
  set_engine("glm") %>%
  fit(expensive ~ Gr_Liv_Area + Overall_Qual, data = ames_class)
...
xgb_spec <- boost_tree(
  ...
  mtry = 0.8,            # Column subsample ratio
...
In [Chapter 12](12-workflows-evaluation.qmd), we'll learn about workflows that combine models with preprocessing and evaluation metrics.
