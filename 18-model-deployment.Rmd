...
production_recipe <- recipe(Sale_Price ~ Gr_Liv_Area + Overall_Qual + 
...
# Evaluate
test_predictions <- predict(production_fit, ames_test) %>%
...
# Verify it works
test_pred <- predict(loaded_model, ames_test %>% slice(1:5))
print(test_pred)
...
model_board %>%
  pin_write(
    ...
    metadata = list(
      ...
      features = c("Gr_Liv_Area", "Overall_Qual", "Year_Built", 
...
#* @param Overall_Qual:numeric Overall quality (1-10)
...
#* @post /predict
function(Gr_Liv_Area, Overall_Qual, Year_Built, Neighborhood, Total_Bsmt_SF) {
...
#* @post /predict
function(req, Gr_Liv_Area, Overall_Qual, Year_Built, Neighborhood, Total_Bsmt_SF) {
  ...
  tryCatch({
    ...
    if (input_data$Overall_Qual < 1 || input_data$Overall_Qual > 10) {
      stop("Overall_Qual must be between 1 and 10")
    }
        ...
        fluidRow(
          box(
            ...
            tags$ul(
              ...
              tags$li("Overall Quality (Overall_Qual)"),
...
production_sample <- ames_test %>%
  mutate(
    # Simulate data drift
    Gr_Liv_Area = Gr_Liv_Area * runif(n(), 0.9, 1.1),
    Year_Built = Year_Built + sample(-5:5, n(), replace = TRUE)
  )
...
training_sample <- ames_train %>%
  select(Gr_Liv_Area, Overall_Qual, Year_Built, Neighborhood, Total_Bsmt_SF) %>%
  slice_sample(n = 200)
...
# Run A/B test
ab_results <- ab_test_framework(production_fit, alternative_model, ames_test)
