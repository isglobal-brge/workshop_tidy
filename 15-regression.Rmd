...
ames_simple <- ames %>%
  select(Sale_Price, Gr_Liv_Area, Year_Built, Overall_Qual) %>%
  drop_na()

# Fit model
ames_lm <- lm(Sale_Price ~ Gr_Liv_Area + Year_Built + Overall_Qual, 
              data = ames_simple)
...
# Recipe with more feature engineering
elastic_recipe <- recipe(Sale_Price ~ ., data = ames_train) %>%
  step_rm(contains("_Condition")) %>%
  step_nzv(all_predictors()) %>%
  step_impute_median(all_numeric_predictors()) %>%
  step_impute_mode(all_nominal_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
...
# Compare solutions
print("Original model R-squared:", summary(problem_fit)$r.squared)
print("Fixed model R-squared:", summary(fixed_fit1)$r.squared)
```
...
predictions_ex <- nonlinear_ex %>%
  mutate(
    linear = predict(linear_ex),
    polynomial = predict(poly_ex),
    spline = predict(spline_ex),
    rf = predict(rf_ex)$.pred
  ) %>%
...
In [Chapter 16](16-ensemble-methods.qmd), we'll explore ensemble methods in depth, learning how to combine multiple models for superior performance.
