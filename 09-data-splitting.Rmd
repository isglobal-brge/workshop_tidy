...
fold_assignments <- map_df(1:5, function(fold) {
  ames_train %>%
    mutate(
      row_id = row_number(),
      fold = fold,
      in_assessment = row_id %in% assessment(ames_cv$splits[[fold]])$row_id
    )
}) %>%
...
simple_recipe <- recipe(Sale_Price ~ Gr_Liv_Area + Overall_Qual, data = ames_train)
...
rolling_summary <- map_df(1:length(rolling_splits$splits), function(i) {
  split <- rolling_splits$splits[[i]]
  tibble(
    split = i,
    train_size = nrow(analysis(split)),
    test_size = nrow(assessment(split))
  )
})
...
models_to_compare <- list(
  simple = recipe(Sale_Price ~ Gr_Liv_Area + Overall_Qual, data = train_set),
  moderate = recipe(Sale_Price ~ Gr_Liv_Area + Overall_Qual + Year_Built + 
                   Neighborhood, data = train_set),
  complex = recipe(Sale_Price ~ ., data = train_set)
)
...
ames_subset <- ames %>%
  select(Sale_Price, Gr_Liv_Area, Overall_Qual, Year_Built, Lot_Area) %>%
  slice_sample(n = 500)
...
In [Chapter 10](10-feature-engineering.qmd), we'll explore feature engineering with recipes, learning how to transform raw data into model-ready features.
