...
ames_recipe <- recipe(Sale_Price ~ Gr_Liv_Area + Overall_Cond + Neighborhood + 
...
recipe_simple <- recipe(Sale_Price ~ Gr_Liv_Area + Overall_Cond, 
                       data = ames_train) %>%
  step_log(Sale_Price)

recipe_normalized <- recipe(Sale_Price ~ Gr_Liv_Area + Overall_Cond + Year_Built, 
                           data = ames_train) %>%
  step_log(Sale_Price) %>%
  step_normalize(all_numeric_predictors())
...
# Use in evaluation
custom_metrics <- yardstick::metric_set(yardstick::rmse, yardstick::mape_threshold)
...
preproc_minimal <- recipe(Sale_Price ~ Gr_Liv_Area + Overall_Cond, 
                         data = ames_train)

preproc_standard <- recipe(Sale_Price ~ Gr_Liv_Area + Overall_Cond + Year_Built, 
                          data = ames_train) %>%
  step_normalize(all_numeric_predictors())

preproc_complex <- recipe(Sale_Price ~ Gr_Liv_Area + Overall_Cond + Year_Built + 
                         Neighborhood + Total_Bsmt_SF, data = ames_train) %>%
  step_log(Sale_Price) %>%
  step_normalize(all_numeric_predictors()) %>%
  step_dummy(all_nominal_predictors()) %>%
  step_interact(terms = ~ Gr_Liv_Area:Overall_Cond)
