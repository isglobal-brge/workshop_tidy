...
ames_simple <- ames_train %>%
  select(Sale_Price, Gr_Liv_Area, Overall_Qual, Year_Built, Neighborhood) %>%
  slice_sample(n = 500)
```
...
# Linear model
lm_spec <- linear_reg(penalty = 0.01, mixture = 0.5) %>%
  set_engine("glmnet")
...
# Initialize stack
model_stack <- stacks() %>%
  add_candidates(lm_workflow %>% 
                  fit_resamples(ames_folds, 
                               control = control_resamples(save_pred = TRUE,
                                                          save_workflow = TRUE))) %>%
  add_candidates(rf_workflow %>%
                  tune_grid(ames_folds,
                           grid = 5,
                           control = control_grid(save_pred = TRUE,
                                                save_workflow = TRUE))) %>%
  add_candidates(xgb_workflow %>%
                  tune_grid(ames_folds,
                           grid = 5,
                           control = control_grid(save_pred = TRUE,
                                                save_workflow = TRUE))) %>%
  add_candidates(knn_workflow %>%
                  tune_grid(ames_folds,
                           grid = 5,
                           control = control_grid(save_pred = TRUE,
                                                save_workflow = TRUE)))
...
blended_stack <- model_stack %>%
  blend_predictions(penalty = 10^seq(-3, 0, length.out = 10))

# See which models contribute
autoplot(blended_stack, type = "weights") +
  labs(
    title = "Model Weights in Stack",
    subtitle = "Different models contribute different amounts"
  )
...
stack_pred <- predict(fitted_stack, ames_test)
stack_rmse <- sqrt(mean((ames_test$Sale_Price - stack_pred$.pred)^2))

cat("Stacked Model RMSE:", stack_rmse, "\n")

# Compare with individual models
individual_rmse <- c(
  "Linear" = sqrt(mean((ames_test$Sale_Price - 
                       predict(lm_workflow %>% fit(ames_train), ames_test)$.pred)^2)),
  "Stack" = stack_rmse
)

cat("\nImprovement over best individual:", 
    round((min(individual_rmse[-length(individual_rmse)]) - stack_rmse) / 
          min(individual_rmse[-length(individual_rmse)]) * 100, 2), "%\n")
```
...
predictions <- map_dfc(models, ~ predict(., class_test, type = "prob")) %>%
...
# Fit and evaluate
diverse_fits <- map(diverse_models, ~ workflow() %>%
                     add_recipe(class_recipe) %>%
                     add_model(.) %>%
                     fit(class_train))
...
complex_fit <- workflow() %>%
  add_recipe(recipe(residual ~ ., data = difficult_cases) %>%
            select(-Sale_Price, -simple_pred, -is_difficult)) %>%
...
# Fit all base models
base_fits <- map(base_models, ~ workflow() %>%
...
val_fits <- map(base_models, ~ workflow() %>%
                 add_recipe(custom_recipe) %>%
                 add_model(.) %>%
                 fit(val_train))
...
In [Chapter 17](17-unsupervised-learning.qmd), we'll explore unsupervised learning techniques for discovering patterns without labels.
