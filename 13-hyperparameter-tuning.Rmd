...
simple_recipe <- recipe(Sale_Price ~ Gr_Liv_Area + Overall_Qual + Year_Built + 
...
tune_and_evaluate <- function(split) {
  ...
  fold_recipe <- recipe(Sale_Price ~ Gr_Liv_Area + Overall_Qual + Year_Built, 
                       data = train_data) %>%
    step_log(Sale_Price) %>%
    step_normalize(all_numeric_predictors())
...
simple_xgb_workflow <- workflow() %>%
  add_recipe(recipe(Sale_Price ~ Gr_Liv_Area + Overall_Qual, data = ames_train)) %>%
  add_model(
...
In [Chapter 16](16-ensemble-methods.qmd), we'll explore ensemble methods that combine multiple models for superior performance.
